...
file_handler = logging.FileHandler(filename='./logs_save/run.log')
...
def valid(device, local_rank, hp, model, writer, test_loader, ep):
    # Validation!
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        with torch.no_grad():
            ...
            test_epoch_loss += eval_loss.item()
    ...
    writer.add_scalar("test/loss", scalar_value=test_epoch_loss,global_step=ep)
    writer.add_scalar("test/accuracy", scalar_value=accuracy, global_step=ep)
    return accuracy
...
def train(local_rank, args, hp, model):
    ...
    losses = AverageMeter()
    global_iters, best_acc = 0, 0

    loss_fct = torch.nn.CrossEntropyLoss()
    train_epoch = 200

    for ep in range(1, train_epoch+1):
        loss_epoch = 0
        model.train()
        ...
        #                       disable=local_rank not in [-1, 0])
        epoch_iterator = tqdm(train_loader,
                              desc="Training (X / X iters) (loss=X.X)",
                              bar_format="{l_bar}{r_bar}",
                              dynamic_ncols=True,
                              disable=local_rank not in [-1, 0])
        for iter, batch in enumerate(epoch_iterator):
            batch = tuple(t.to(device) for t in batch)
            x, y = batch
            logits = model(x)
            ...
            if (iter + 1) % hp.train.accum_grad == 0:
                ...
                loss_epoch += losses.val

                epoch_iterator.set_description(
                    "Training (%d / %d) (%d / %d iters) (loss=%2.5f)" % (ep, train_epoch, iter+1, len(epoch_iterator), loss_epoch/(iter+1))
                )
                if local_rank in [-1, 0]:
                    writer.add_scalar("train/loss", scalar_value=losses.val, global_step=global_iters)
                    writer.add_scalar("train/lr", scalar_value=scheduler.get_lr()[0], global_step=global_iters)
                    writer.add_scalar("train/lr_optimizer", scalar_value=optimizer.defaults['lr'], global_step=global_iters)
                # if global_iters % hp.train.valid_step == 0 and local_rank in [-1, 0]:
                #     accuracy = valid(device, local_rank, hp, model, writer, test_loader, global_iters)
                #     if best_acc < accuracy:
                #         save_model(args.name, hp.data.outdir, model)
                #         best_acc = accuracy
                #     model.train()

                # if global_iters % t_total == 0:
                #     break
        if local_rank in [-1, 0]:
            accuracy = valid(device, local_rank, hp, model, writer, test_loader, ep)
            if best_acc < accuracy:
                save_model(args.name, hp.data.outdir, model)
                best_acc = accuracy
            model.train()
        writer.add_scalar('train_epoch/loss', scalar_value=loss_epoch/len(train_loader), global_step=ep)
        # writer.add_scalar('train_epoch/lr', scalar_value=scheduler.get_lr()[0], global_step=ep)
        losses.reset()
        # if global_iters % t_total == 0:
        #     break
