...
def decom_resnet50():
    # pretrained=True: ImageNet 预训练 backbone 网络
    ...
    features_top = torch.nn.Sequential(model.layer4, model.avgpool)
    classifier = torch.nn.Sequential(model.fc)

    for layer in features_base[:5]:  # 冻结 conv1 和conv2_x
...
def mask_rcnn_resnet50():
    ...
    # 使用 coco 训练的 Mask R-CNN 的 backbone 网络参数，初始化 Faster R-CNN 的 backbone 网络
    model_dict = model.state_dict()
    checkpoint_dict = torch.load(opt.load_path)
    checkpoint_dict = {
        k.replace('backbone.body.', ''): v for k, v in checkpoint_dict.items()
    }
    backbone_dict = {k: v for k, v in checkpoint_dict.items() if k in model_dict.keys()}
    ...
    for layer in features_base[:5]:  # 冻结 conv1 和 conv2_x
        for p in layer.parameters():
            p.requires_grad = False
...
class FasterRCNNResNet50(FasterRCNN):
    ...
    def __init__(
        self, n_fg_class=opt.class_num, ratios=[0.5, 1, 2], anchor_scales=[8, 16, 32]
    ):
        # ImageNet 预训练 backbone 网络
        features_base, features_top, classifier = decom_resnet50()

        # 使用 coco 训练的 Mask R-CNN 的 backbone 网络参数，初始化 Faster R-CNN 的 backbone 网络
        # features_base, features_top, classifier = mask_rcnn_resnet50()
