...
class FasterRCNN(nn.Module):
    ...
    def _new_suppress(self, raw_cls_bbox, raw_prob):
        bbox = list()
        label = list()
        score = list()
        # skip cls_id = 0 because it is the background class
    ...
    def predict(self, imgs,sizes=None,visualize=False):
        ...
        for img, size in zip(prepared_imgs, sizes):
            ...
            bbox, label, score = self._suppress(cls_bbox, prob)  # 原来为_suppress
            bboxes.append(bbox)
            labels.append(label)
            scores.append(score)
    ...
    @nograd
    def show_rois(self, imgs, output_path='', sizes=None,visualize=False):
        """Detect objects from images.
        ...
        for img, size in zip(prepared_imgs, sizes):
            ...
            bbox, label, score = self._new_suppress(cls_bbox, prob)  # 原来为_suppress
            bboxes.append(bbox)
            labels.append(label)
            scores.append(score)
